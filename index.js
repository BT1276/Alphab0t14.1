...
await limitAdd(sender, limit)
		...
		if (!isQuotedSticker) return reply('```Reply stc nya```')
		            ...
		            anu = args.join(' ').split('|')
		            satu = anu[0] !== '' ? anu[0] : `BT1276`
		            dua = typeof anu[1] !== 'undefined' ? anu[1] : `BT BOT WA`
			...
			case 'sticker': case 'stikerin':
				case 'stiker': case 's':{
						...
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							...
							exif.create('BT1276', 'BT BOT WA', `stickwm_${sender}`)
							//ran = getRandom('.webp')
                        await ffmpeg(`./${media}`)
                            .input(media)
